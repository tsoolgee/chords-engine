# אקורדים — זיהוי מילים ואקורדים לשירים בעברית

תוכנת Windows בעברית: פותחים שיר, והתוכנה מזהה את המילים ואת האקורדים ומציגה
אותם מסונכרנים עם השמע — המילה והאקורד שמתנגנים כרגע מודגשים תוך כדי הנגינה.
אפשר לתקן הכול ידנית, להעביר לסולם אחר, לשים קאפו, לתרגל בלופ ובהאטה, ולייצא.

התוכנה בנויה משתי שכבות: **מנוע** ב-Python ששרת HTTP מקומי חושף אותו,
ו**ממשק** ב-HTML שרץ בתוך חלון WebView2. הממשק הנוכחי הוא אב-טיפוס עובד,
שיוחלף בעיצוב של גרפיקאי — ולכן כל מה שהוא עושה מתועד:

- מסמך האב של המוצר: [docs/SPEC.md](docs/SPEC.md)
- אפיון הממשק לגרפיקאי: [docs/UI_SPEC.md](docs/UI_SPEC.md)
...
| תגיות בקבצי שמע | mutagen | GPL-2.0+ | הטמעת המילים והאקורדים בקובץ המקורי |
...
הגרסה האחרונה: **[Chords.exe](https://github.com/tsoolgee/chords-engine/releases/latest)** — קובץ אחד, בלי התקנה,
כולל מודל העברית. ההפעלה הראשונה לוקחת כדקה (פריסה חד-פעמית), אחר כך נפתח מיד.
דרישות: Windows 10/11 64-bit ו-WebView2 (מגיע עם ווינדוס 11).

## נגן חיצוני (HTML אחד)

[`player/index.html`](player/index.html) הוא נגן עצמאי — קובץ אחד, בלי שרת ובלי אינטרנט.
גוררים אליו קובץ שיוצא מהתוכנה ("ייצוא → 🎧 קובץ השמע + מילים ואקורדים") והוא קורא מתוך
התגיות את המילים ואת האקורדים ומציג אותם מסונכרנים: אקורד נוכחי והבא, הדגשת שורה ומילה,
גלילה אוטומטית, טרנספוזיציה, שינוי מהירות ולופ A-B.
הוא קורא ID3 (MP3), אטומים של MP4/M4A ו-VorbisComment (FLAC) — הכול ב-JavaScript, בלי ספריות.
אפשר להשתמש בו כמו שהוא, או כדוגמה לנגן משלך.
...
- **עריכה:** מצב העריכה לא משנה את מראה הדף. מקלידים מילים ישר בשורה, ולוחצים על אקורד כדי להקליד אותו במקומו. `[` מוסיף אקורד בסמן, Enter מפצל שורה, Ctrl+Z מבטל, והשמירה אוטומטית.
